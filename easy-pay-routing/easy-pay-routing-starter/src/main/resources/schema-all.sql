...
-- 商户应用支付渠道表 t_merchant_payment_channel
DROP TABLE IF EXISTS t_merchant_payment_channel;
CREATE TABLE `t_merchant_payment_channel`
(
    `application_channel_id` BIGINT UNSIGNED PRIMARY KEY AUTO_INCREMENT COMMENT '商户应用渠道ID',
    ...
    `created_at`          TIMESTAMP(3)   NOT NULL DEFAULT CURRENT_TIMESTAMP(3) COMMENT '创建时间',
    `updated_at`          TIMESTAMP(3)   NOT NULL DEFAULT CURRENT_TIMESTAMP(3) ON UPDATE CURRENT_TIMESTAMP(3) COMMENT '更新时间'
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='商户应用支付渠道表';
